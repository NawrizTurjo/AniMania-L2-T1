...
export default function Episode({ toggleRerender, setProgress }) {
  ...
  const getAnime = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:3000/watch/anime/episodes/${id}/episode/${id2}`
      );
      setAnime(res.data[0]);
      setLoading(false);
  ...
  const handleSubmitReplies = async (e, parentId) => {
    ...
    try {
      await axios.post(
        `http://localhost:3000/watch/anime/episodes/${id}/episode/${id2}/reply`,
        {
          id: id,
          id2: id2,
          reply: reply,
  ...
  const handleSubmitLikes = async (e, commentId) => {
    ...
    try {
      ...
      if (isLiked) {
        await axios.post(`http://localhost:3000/reactionLremove`, {
          email: email,
          commentId: commentId,
        });
  ...
  const handleSubmitDisLikes = async (e, commentId) => {
    ...
    try {
      ...
      if (isDisliked) {
        await axios.post(`http://localhost:3000/reactionDremove`, {
          email: email,
          commentId: commentId,
        });
  ...
  async function updateHistory() {
    try {
      const res = await axios.put(
        `http://localhost:3000/updateHistory`,
        {
          email: email,
          anime_id: id,
          episode_no: id2,
  ...
  const fetchUserLikes = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/getlike?email=${email}`
      );
  ...
  useEffect(() => {
    // Fetch user likes and dislikes when the component mounts
    fetchUserLikes();
    fetchUserDislikes();
  }, []);
  ...
  const renderPagination = () => {
    ...
    for (let i = 1; i <= anime.number_of_episodes; i++) {
      paginationButtons.push(
        <Link
          key={i}
          to={`/watch/anime/episodes/${id}/episode/${i}`}
          className="col-auto"
        >
            ...
            <div className="card-body">
              ...
              <ul className="list-group list-group-flush">
                <li className="list-group-item">
                  <strong>Episode Number:</strong> {anime.episode_no}
                </li>
                <li className="list-group-item">
                  <strong>Release Date:</strong> {anime.release_date}
                </li>
                <li className="list-group-item">
                  <strong>Availability:</strong> {anime.availability}
                </li>
                <li className="list-group-item">
                  <strong>Anime ID:</strong> {anime.anime_id}
                </li>
                <li className="list-group-item">
                  <a href={anime.streaming_sites}>
                    <strong>Streaming Sites</strong>
                  </a>
                </li>
                <li className="list-group-item">
                  <strong>Length:</strong> {anime.LENGTH}
                </li>
              </ul>
    ...
    <Typography variant="body1">Dislikes: {dislikes[comment.comment_id]}</Typography>
                ...
                {email === comment.email ? (
                  <Box sx={{ mt: 2, textAlign: "center" }}>
                    ...
                    <Button
                      ...
                      color="primary"
                      onClick={() =>
                        handleUpdateReview(comment.comment_id, editedText)
                      }
                ...
                {replyIndex === index ? (
                  <div>
                    <form
                      onSubmit={(e) =>
                        handleSubmitReplies(e, comment.comment_id)
                      }
                    >
                      <div
